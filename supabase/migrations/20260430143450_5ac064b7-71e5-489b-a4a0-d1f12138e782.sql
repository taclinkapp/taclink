  ...
  v_payload := jsonb_build_object(
    ...
    'context', jsonb_build_object(
      ...
      'policy', jsonb_build_object(
        ...
        'note', 'TacLink issues CASH refunds to the student''s original payment method via Stripe within 48 hours. There is NO in-app credit system. The student''s grace window is tiered (72/48/24/0 hours) based on how far ahead they booked — see booking.cancellation_cutoff_hours. Within window = full cash refund of $25 + 10% deposit. After window = no refund. Instructor-fault cases (no-show, cancel) = full cash refund regardless of timing. The 90% paid in person is never refundable by TacLink. Never promise in-app credit.'
      )
    )
  );
