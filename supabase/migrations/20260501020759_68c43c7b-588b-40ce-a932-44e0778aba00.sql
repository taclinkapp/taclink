...
BEGIN
  ...
  v_requested_role := lower(trim(COALESCE(NEW.raw_user_meta_data->>'role', 'student')));
  v_role := CASE
    WHEN v_requested_role = 'instructor' THEN 'instructor'::public.app_role
    WHEN v_requested_role = 'admin' THEN 'student'::public.app_role
