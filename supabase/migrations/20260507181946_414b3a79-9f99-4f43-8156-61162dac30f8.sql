DROP FUNCTION IF EXISTS public.instructor_cancel_course(uuid, text);

CREATE OR REPLACE FUNCTION public.instructor_cancel_course(_course_id uuid, _reason text DEFAULT NULL::text)
 RETURNS TABLE(course_id uuid, hours_before_start numeric, was_timely boolean, bookings_refunded integer, total_refunded_cents integer, instructor_forfeited_cents integer, listing_fee_refunded_cents integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  v_listing_refunded integer := 0;
  v_listing record;
BEGIN
  IF v_caller IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;

  SELECT c.id, c.instructor_id, c.starts_at, c.status, c.title
    INTO v_course FROM public.courses c WHERE c.id = _course_id FOR UPDATE;
  ...
  END IF;
  IF v_course.status = 'cancelled' THEN RAISE EXCEPTION 'Course is already cancelled'; END IF;

  IF v_course.starts_at IS NULL THEN
    v_hours := NULL; v_timely := true;
  ELSE
  ...
  FOR v_booking IN
    SELECT b.id, b.student_id, b.platform_fee_cents, b.deposit_amount_cents,
     ...
     WHERE b.course_id = _course_id AND b.status NOT IN ('cancelled')
  LOOP
    ...
    IF v_refund_amount > 0 THEN
      INSERT INTO public.refunds (
        ...
        student_cash_refund_cents, refund_method, hours_before_course, notes, auto_issued
      ) VALUES (
    ...
    VALUES (
      v_booking.student_id::text, 'course_cancelled',
  ...
  -- Refund the listing fee on timely cancels regardless of bookings.
  IF v_timely THEN
    FOR v_listing IN
      SELECT ic.id, ic.amount_cents FROM public.instructor_charges ic
       WHERE ic.course_id = _course_id AND ic.charge_type = 'listing_fee' AND ic.status = 'charged'
    LOOP
      UPDATE public.instructor_charges
         SET status = 'refunded',
             refundable = true,
             note = COALESCE(note || ' | ', '') || 'Refunded on timely instructor cancellation (' || to_char(now(), 'YYYY-MM-DD') || ').'
       WHERE id = v_listing.id;
      v_listing_refunded := v_listing_refunded + COALESCE(v_listing.amount_cents, 0);
    END LOOP;

    IF v_listing_refunded > 0 THEN
      INSERT INTO public.notifications (recipient_id, type, title, body, link)
      VALUES (
        v_course.instructor_id::text, 'listing_fee_refunded',
        'Listing fee refunded',
        'Your $' || to_char(v_listing_refunded / 100.0, 'FM999990.00') ||
          ' listing fee for "' || v_course.title || '" has been refunded (cancelled with 48+ hours notice).',
        '/instructor/dashboard'
      );
    END IF;
  END IF;

  UPDATE public.courses SET status = 'cancelled', updated_at = now() WHERE id = _course_id;
  ...
  RETURN QUERY SELECT _course_id, v_hours, v_timely, v_count, v_total, v_forfeited, v_listing_refunded;
END;
$function$;