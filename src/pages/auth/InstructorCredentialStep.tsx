...
import { Upload, ShieldCheck, Camera, ImageIcon, FileText, X } from 'lucide-react';
...
/**
 * Guest credential step. Holds the credential file in the in-memory draft
 * — actual upload happens at the final step after the auth account is
 * created.
 */
const InstructorCredentialStep = () => {
  ...
  useEffect(() => {
    ...
    if (draft.credentialFile) setFile(draft.credentialFile);
    if (draft.credentialType) setCredentialType(draft.credentialType);
  }, [nav]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateInstructorDraft({
      credentialType,
      credentialDisplayName: options.find((o) => o.id === credentialType)?.label ?? credentialType,
      credentialFile: file,
    });
    nav('/auth/instructor/policy', { replace: true });
  };
  ...
  return (
    <div className="relative min-h-screen bg-background overflow-hidden">
      ...
      <div className="relative z-10">
        <PageHeader title="Verify Credentials" back backTo="/auth/instructor/plan" />
        <div className="max-w-md mx-auto px-6 py-6">
          <InstructorDraftProgress
            current="credential"
            completed={{ account: true, plan: true }}
            className="mb-6"
          />
          <div className="flex items-start gap-3 mb-6">
            <ShieldCheck className="h-6 w-6 text-primary shrink-0 mt-0.5" />
            <p className="text-sm text-muted-foreground">
              Upload a document proving your qualifications. AI reviews submissions within 1 hour after your account is created.
            </p>
          </div>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              {file ? (
                <div className="tactical-card p-4 flex items-center gap-3">
                  ...
                  <button type="button" onClick={() => setFile(null)} className="h-8 w-8 grid place-items-center rounded-md hover:bg-muted transition" aria-label="Remove file">
                    <X className="h-4 w-4" />
                  </button>
                </div>
              ) : pickerOpen ? (
                <div className="tactical-card p-4 space-y-2">
                  <button type="button" onClick={() => cameraInputRef.current?.click()} className="w-full flex items-center gap-3 p-3 rounded-lg hover:bg-primary/10 transition text-left">
                    <Camera className="h-5 w-5 text-primary" /><span className="text-sm font-semibold">Take Photo</span>
                  </button>
                  <button type="button" onClick={() => galleryInputRef.current?.click()} className="w-full flex items-center gap-3 p-3 rounded-lg hover:bg-primary/10 transition text-left">
                    <ImageIcon className="h-5 w-5 text-primary" /><span className="text-sm font-semibold">Choose from Photos</span>
                  </button>
                  <button type="button" onClick={() => fileInputRef.current?.click()} className="w-full flex items-center gap-3 p-3 rounded-lg hover:bg-primary/10 transition text-left">
                    <FileText className="h-5 w-5 text-primary" /><span className="text-sm font-semibold">Choose PDF</span>
                  </button>
                  <button type="button" onClick={() => setPickerOpen(false)} className="w-full text-xs text-muted-foreground py-2">Cancel</button>
                </div>
              ) : (
                <button type="button" onClick={() => setPickerOpen(true)} className="w-full h-32 tactical-card border-dashed border-primary/40 flex flex-col items-center justify-center gap-2 hover:border-primary transition">
            ...
            <Button type="submit" disabled={!file} className="w-full h-12 bg-primary text-primary-foreground hover:bg-primary/90 font-bold disabled:opacity-50">
              Continue
            </Button>
          </form>
        </div>
...
export default InstructorCredentialStep;
