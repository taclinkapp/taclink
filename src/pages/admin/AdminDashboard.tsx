    ...
    <>
      ...
      <div className="p-4 sm:p-8">
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 mb-6 sm:mb-8">
          {stats.map((s) => (
            <div key={s.label} className={`tactical-card p-4 sm:p-5 ${s.primary ? 'border-primary/40' : ''}`}>
              ...
              <div className={`text-2xl sm:text-3xl font-black break-words ${s.primary ? 'text-primary' : ''}`}>{s.value}</div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
          <Button variant="outline" className="h-14 sm:h-16 bg-card border-border font-semibold justify-start gap-3"><AlertTriangle className="text-destructive" />Suspend User</Button>
          <Button variant="outline" className="h-14 sm:h-16 bg-card border-border font-semibold justify-start gap-3"><Bell className="text-primary" />Broadcast Notification</Button>
          <Button variant="outline" className="h-14 sm:h-16 bg-card border-border font-semibold justify-start gap-3"><Calendar className="text-primary" />Change Launch Date</Button>
        </div>

        <div className="mt-6 sm:mt-8 tactical-card p-4 sm:p-6">
          ...
          <div className="space-y-3 text-sm">
            {[
              ...
              <div key={i} className="flex items-start sm:items-center gap-3 py-2 border-b border-border last:border-0 flex-wrap sm:flex-nowrap">
                <div className="h-2 w-2 rounded-full bg-primary mt-2 sm:mt-0 shrink-0" />
                <span className="text-muted-foreground min-w-0 flex-1"><span className="text-foreground font-semibold">{a.who}</span> {a.what} <span className="text-primary font-semibold">{a.target}</span></span>
                <span className="ml-auto text-xs text-muted-foreground shrink-0">{a.when}</span>
              </div>
            ))}
          </div>
        </div>
